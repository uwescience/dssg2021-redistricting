...
Adapted from MGGG VRA modeling code - https://github.com/mggg/VRA_ensembles/tree/master/TX
"""
import csv
import os
import shutil
...
def demo_percents(partition):
    hisp_pct = {k: partition["HCVAP"][k] / partition["CVAP"][k] for k in
                partition["HCVAP"].keys()}
    black_pct = {k: partition["BCVAP"][k] / partition["CVAP"][k] for k in
                 partition["BCVAP"].keys()}
    white_pct = {k: partition["WCVAP"][k] / partition["CVAP"][k] for k in
                 partition["WCVAP"].keys()}
    return hisp_pct, black_pct, white_pct


def centroids(partition):
    CXs = {k: partition["Sum_CX"][k] / len(partition.parts[k]) for k in
           list(partition.parts.keys())}
    CYs = {k: partition["Sum_CY"][k] / len(partition.parts[k]) for k in
           list(partition.parts.keys())}
...
initial_partition = GeographicPartition(graph=graph, assignment=start_map,
...
proposal = partial(
    recom, pop_col=tot_pop, pop_target=ideal_population,
    epsilon= pop_tol, node_repeats=3
)

# constraints ############################################
popbound = constraints.within_percent_of_ideal_population(initial_partition,
                                                          pop_tol,
                                                          pop_key='population')
...
store_plans["GEOID"] = [state_gdf_geoid.iloc[i][0] for i in
                        store_plans["Index"]]
map_metric = pd.DataFrame(
    columns=["Latino_state", "Black_state", "Distinct_state", \
...
if record_statewide_modes:
    final_state_prob_df = pd.DataFrame(columns=range(num_districts),
                                       index=list(range(store_interval)))
...
if record_district_mode:
    final_dist_prob_df = pd.DataFrame(columns=range(num_districts),
                                      index=list(range(store_interval)))
...
sen18_df = pd.DataFrame(columns=range(num_districts),
                        index=list(range(store_interval)))
gov18_df = pd.DataFrame(columns=range(num_districts),
                        index=list(range(store_interval)))
...
for step in chain.with_progress_bar():
    final_state_prob, final_equal_prob, final_dist_prob = step[
        "final_elec_model"]

    total_hisp_final_state, total_black_final_state, total_distinct_state = effective_districts(
        final_state_prob)
    total_hisp_final_equal, total_black_final_equal, total_distinct_equal = effective_districts(
        final_equal_prob)
    total_hisp_final_dist, total_black_final_dist, total_distinct_dist = effective_districts(
        final_dist_prob)

    map_metric.loc[step_Num] = [total_hisp_final_state, total_black_final_state,
                                total_distinct_state, \
                                total_hisp_final_equal, total_black_final_equal,
                                total_distinct_equal, \
                                total_hisp_final_dist, total_black_final_dist,
                                total_distinct_dist, \
                                step["num_cut_edges"],
                                step["num_county_splits"]]

    state_gdf['current'] = state_gdf.index.map(step.assignment.to_dict())
    num_districts_per_county.append(
        state_gdf.groupby('CNTY_x')['current'].nunique())

    # saving all data at intervals
    if step_Num % store_interval == 0 and step_Num > 0:
        store_plans.to_csv(DIR + "outputs/store_plans_{}.csv".format(run_name),
                           index=False)
        # store data and reset data frames
        if step_Num == store_interval:
            print("store data, step", step_Num)
            pres16_df.to_csv(DIR + "outputs/pres16_df_{}.csv".format(run_name),
                             index=False)
            pres16_df = pd.DataFrame(columns=range(num_districts),
                                     index=list(range(store_interval)))
            pres12_df.to_csv(DIR + "outputs/pres12_df_{}.csv".format(run_name),
                             index=False)
            pres12_df = pd.DataFrame(columns=range(num_districts),
                                     index=list(range(store_interval)))
            sen18_df.to_csv(DIR + "outputs/sen18_df_{}.csv".format(run_name),
                            index=False)
            sen18_df = pd.DataFrame(columns=range(num_districts),
                                    index=list(range(store_interval)))
            gov18_df.to_csv(DIR + "outputs/gov18_df_{}.csv".format(run_name),
                            index=False)
            gov18_df = pd.DataFrame(columns=range(num_districts),
                                    index=list(range(store_interval)))

            hisp_prop_df.to_csv(
                DIR + "outputs/hisp_prop_df_{}.csv".format(run_name),
                index=False)
            hisp_prop_df = pd.DataFrame(columns=range(num_districts),
                                        index=list(range(store_interval)))
            black_prop_df.to_csv(
                DIR + "outputs/black_prop_df_{}.csv".format(run_name),
                index=False)
            black_prop_df = pd.DataFrame(columns=range(num_districts),
                                         index=list(range(store_interval)))
            white_prop_df.to_csv(
                DIR + "outputs/white_prop_df_{}.csv".format(run_name),
                index=False)
            white_prop_df = pd.DataFrame(columns=range(num_districts),
                                         index=list(range(store_interval)))
            ...
            for score_df, score_df_name in zip(score_dfs, score_df_names):
                score_df.to_csv(
                    DIR + "outputs/{}_{}.csv".format(score_df_name, run_name),
                    index=False)
                score_df = pd.DataFrame(columns=range(num_districts),
                                        index=list(range(store_interval)))
            ...
            gov18_df = pd.DataFrame(columns=range(num_districts),
                                    index=list(range(store_interval)))
            hisp_prop_df.to_csv(
                DIR + "outputs/hisp_prop_df_{}.csv".format(run_name), mode='a',
                header=False, index=False)
            hisp_prop_df = pd.DataFrame(columns=range(num_districts),
                                        index=list(range(store_interval)))
            black_prop_df.to_csv(
                DIR + "outputs/black_prop_df_{}.csv".format(run_name), mode='a',
                header=False, index=False)
            black_prop_df = pd.DataFrame(columns=range(num_districts),
                                         index=list(range(store_interval)))
            white_prop_df.to_csv(
                DIR + "outputs/white_prop_df_{}.csv".format(run_name), mode='a',
                header=False, index=False)
            white_prop_df = pd.DataFrame(columns=range(num_districts),
                                         index=list(range(store_interval)))
    ...
    hisp_prop_df.loc[step_Num % store_interval] = [value for _, value in
    ...
    black_prop_df.loc[step_Num % store_interval] = [value for _, value in
                                                    sorted(zip(keys, values))]

    white_prop_data = step["demo_percents"][2]
    keys = list(white_prop_data.keys())
    values = list(white_prop_data.values())
    white_prop_df.loc[step_Num % store_interval] = [value for _, value in
                                                    sorted(zip(keys, values))]

    order = [int(x) for x in step.parts]
    percents = {}
    for elec in elections_track:
        percents[elec.name] = dict(
            zip(order, step[elec.name].percents("Democratic")))

    keys = list(percents["PRES16"].keys())
    values = list(percents["PRES16"].values())
    pres16_df.loc[step_Num % store_interval] = [value for _, value in
                                                sorted(zip(keys, values))]

    keys = list(percents["PRES12"].keys())
    values = list(percents["PRES12"].values())
    pres12_df.loc[step_Num % store_interval] = [value for _, value in
                                                sorted(zip(keys, values))]

    keys = list(percents["SEN18"].keys())
    values = list(percents["SEN18"].values())
    sen18_df.loc[step_Num % store_interval] = [value for _, value in
                                               sorted(zip(keys, values))]

    keys = list(percents["GOV18"].keys())
    values = list(percents["GOV18"].values())
    gov18_df.loc[step_Num % store_interval] = [value for _, value in
                                               sorted(zip(keys, values))]

    if record_statewide_modes:
        final_state_prob_df.loc[step_Num % store_interval] = list(
            final_state_prob.values())
        final_equal_prob_df.loc[step_Num % store_interval] = list(
            final_equal_prob.values())

    if record_district_mode:
        final_dist_prob_df.loc[step_Num % store_interval] = list(
            final_dist_prob.values())

        # store plans
    if (step_Num - last_step_stored) == store_interval or step_Num == 0:
        last_step_stored = step_Num
        store_plans["Map{}".format(step_Num)] = store_plans["Index"].map(
...
store_plans.to_csv(DIR + "outputs/store_plans_{}.csv".format(run_name),
                   index=False)
hisp_prop_df.to_csv(DIR + "outputs/hisp_prop_df_{}.csv".format(run_name),
                    mode='a', header=False, index=False)
black_prop_df.to_csv(DIR + "outputs/black_prop_df_{}.csv".format(run_name),
                     mode='a', header=False, index=False)
white_prop_df.to_csv(DIR + "outputs/white_prop_df_{}.csv".format(run_name),
                     mode='a', header=False, index=False)
pres16_df.to_csv(DIR + "outputs/pres16_df_{}.csv".format(run_name), mode='a',
                 header=False, index=False)
pres12_df.to_csv(DIR + "outputs/pres12_df_{}.csv".format(run_name), mode='a',
                 header=False, index=False)
sen18_df.to_csv(DIR + "outputs/sen18_df_{}.csv".format(run_name), mode='a',
                header=False, index=False)
gov18_df.to_csv(DIR + "outputs/gov18_df_{}.csv".format(run_name), mode='a',
                header=False, index=False)

map_metric.to_csv(DIR + "outputs/map_metric_{}.csv".format(run_name),
                  index=True)

# county split data
_district_count_df = pd.concat(num_districts_per_county, axis=1)
_district_count_df.columns = range(total_steps)
_district_count_df.to_csv(DIR + 'outputs/districts_per_count_{}.csv'.format(run_name),
                          index=True)
...
if total_steps <= store_interval:
    ...
    for score_df, score_df_name in zip(score_dfs, score_df_names):
        score_df.to_csv(
            DIR + "outputs/{}_{}.csv".format(score_df_name, run_name), mode='a',
            header=False, index=False)
